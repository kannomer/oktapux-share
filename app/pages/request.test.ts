...
const { default: RequestPage } = await import('./request.vue')
...
describe('request page', () => {
  ...
  it('warns when a password is required but missing', async () => {
    ...
    await wrapper.get('button').trigger('click')
    const createRequestButtons = wrapper
	.findAll('button')
	.filter(button => button.text() === 'Create request')

	expect(createRequestButtons).toHaveLength(2)
	await createRequestButtons[1]!.trigger('click')
