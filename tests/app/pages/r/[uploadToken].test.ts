...
const { default: ReverseUploadPage } = await import('../../../../app/pages/r/[uploadToken].vue')
...
describe('reverse upload page', () => {
  ...
  it('warns when submitting without files', async () => {
    ...
    ;(wrapper.vm as unknown as { attemptSubmit: () => Promise<void> }).attemptSubmit()
  ...
  it('submits selected files and shows the success state', async () => {
    ...
    await vi.waitFor(() => expect((wrapper.vm as unknown as { fileUploadValue: File[] }).fileUploadValue).toHaveLength(1))
    await (wrapper.vm as unknown as { attemptSubmit: () => Promise<void> }).attemptSubmit()

    await vi.waitFor(() => {
      expect(fetchMock).toHaveBeenCalledWith('/api/reverse/upload-token', expect.objectContaining({ method: 'POST' }))
    })
    expect(wrapper.text()).toContain('Your files were submitted.')
  })
})
