...
vi.mock('../../../server/db/index', () => ({ db: { select: selectMock, update: updateMock } }))
vi.mock('../../../server/db/schema', () => ({ settings: { id: 'id' } }))
...
const { default: patchConfig } = await import('../../../server/api/config.patch')
