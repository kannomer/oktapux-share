...
vi.mock('../../../server/db/index', () => ({ db: dbMock }))
...
it('returns 400 when the request contains no files', async () => {
  ...
  await expect(upload(makeEvent())).rejects.toMatchObject({
    statusCode: 400,
    statusMessage: 'Validation failed. No files exist in collection',
  })
})
...
it('returns 400 when passwordless shares are disabled', async () => {
  ...
  await expect(upload(makeEvent())).rejects.toMatchObject({
    statusCode: 400,
    statusMessage: 'This server requires a password on all Crates',
  })
})
...
it('returns 400 when permanent shares are disabled', async () => {
  ...
  await expect(upload(makeEvent())).rejects.toMatchObject({
    statusCode: 400,
    statusMessage: 'Permanent Crates are disabled on this server',
  })
})
...
it('returns 500 when share creation fails for an unexpected database error', async () => {
  ...
  await expect(upload(makeEvent())).rejects.toMatchObject({
    statusCode: 500,
    statusMessage: 'Failed to create Crate',
  })
})
